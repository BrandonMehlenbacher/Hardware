...
class QFactor(object):
    ...
    guessesMod: this will be implemented later and will use the modified lorentzian function for fitting parameters


    """

    def __init__(self,x_values,y_values,guessesLor = [1,1],guessesMod = [0,0,0,0]):
    ...
    #def fitModLorentz(self):
       # params = curve_fit(self.modLorent,xdata=self._x_values,ydata=self._y_values)
        #return params[0]
    def modLorent(self,a,b,c,d,x):
        return a+(b/(1+((x-c)/d)^2))
    def fitLorentz(self):
        max_arg = self._y_values.index(max(self._y_values)) #finds max value to center the peak
        mod = LorentzianModel(prefix = "L_1")
        pars = mod.make_params()
        pars['L_1center'].set(value = self._x_values[max_arg],min = self._x_values[max_arg]-10, max = self._x_values[max_arg]+10)
        pars['L_1sigma'].set(value = self._sigma,min = .001)
    ...
    def find_peaks(self,height):
        list_peaks = find_peaks(self._y_values,height = 0.1)[0][0]
        return list_peaks
    def getCenter(self):
